...
SET @old = (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'item_display_info_model_mat_res');
SET @sql = IF(@old, CONCAT('RENAME TABLE `item_display_info_model_mat_res` TO `item_display_info_model_mat_res_deleted_', @archive_at, '`'), 'DO 0');
