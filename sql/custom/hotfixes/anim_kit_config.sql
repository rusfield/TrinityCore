...
SET @old = (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'anim_kit_config');
SET @sql = IF(@old, CONCAT('RENAME TABLE `anim_kit_config` TO `anim_kit_config_deleted_', @archive_at, '`'), 'DO 0');
