...
SET @old = (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'creature_display_info_option');
SET @sql = IF(@old, CONCAT('RENAME TABLE `creature_display_info_option` TO `creature_display_info_option_deleted_', @archive_at, '`'), 'DO 0');
...
CREATE TABLE `creature_display_info_option` (
  ...
  `CreatureDisplayInfoExtraID` int unsigned NOT NULL DEFAULT '0',
